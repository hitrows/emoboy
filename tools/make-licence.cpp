...
// signing key pair itself. Ported from the "Not Sure" project's own tool
// (2026-08-21) - see its LICENCE-SPEC.md for the full reasoning.
//
// Two modes:
//
//   make-licence --genkey
...
//       given path (or EMOBOY_PRIVATE_KEY in the environment if --key is
...
//       ~/Private/emoboy-signing.key > licence.txt
...
namespace
{
    void printUsage()
    {
        std::cerr <<
            ...
            "                             (or set EMOBOY_PRIVATE_KEY instead of --key)\n";
    }
}
...
int main (int argc, char* argv[])
{
    ...
    if (args.contains ("--genkey"))
    {
        ...
        // 512 bits: same call as "Not Sure" made - this project's own
        // security bar here is deliberately low, matching JUCE's own
        // historical licensing examples, and keeps applyToValue() (pure
        // BigInteger arithmetic, no hardware acceleration) fast on every
        // plugin construction.
    ...
    if (keyFlag >= 0 && keyFlag + 1 < args.size())
        ...
        keyPath = juce::SystemStats::getEnvironmentVariable ("EMOBOY_PRIVATE_KEY", {});

    if (keyPath.isEmpty())
    {
        std::cerr << "error: no private key - pass --key <path> or set EMOBOY_PRIVATE_KEY\n";
        printUsage();
        return 1;
    }
    ...
    std::cout << emoboy::signLicence (name, email, privateKey) << "\n";
    return 0;
}
