// Exercises signLicence/verifyLicence for real: no file, empty file, random
// bytes, a validly-signed licence, a corrupted signature, a licence signed
// with a different key, a truncated file, and an invalid public key. Ported
// from "Not Sure"'s own notsure-licence-test.cpp (2026-08-21). Uses
...
int main()
{
    ...
    check ("no file: unregistered",
           emoboy::verifyLicence ({}, publicA) == "unregistered");
    ...
    for (int seed = 0; seed < 5; ++seed)
        check ("random bytes (seed " + juce::String (seed) + "): unregistered",
               emoboy::verifyLicence (randomBytesAsString (256, (unsigned) seed), publicA) == "unregistered");

    // 4: a validly-signed licence.
    {
        const auto text = emoboy::signLicence ("Ivan Petrov", "ivan@example.com", privateA);
        check ("valid licence: shows the buyer's name",
               emoboy::verifyLicence (text, publicA) == "licensed to Ivan Petrov");
    }

    // 5: a corrupted signature - flip one hex character in an otherwise-real file.
    {
        auto text = emoboy::signLicence ("Ivan Petrov", "ivan@example.com", privateA);
        ...
        check ("corrupted signature: unregistered, not an error",
               emoboy::verifyLicence (text, publicA) == "unregistered");
    }

    // 6: signed with a different key pair than the one checking it.
    {
        const auto text = emoboy::signLicence ("Ivan Petrov", "ivan@example.com", privateB);
    ...
    // 7: a licence text with only two lines (truncated mid-field).
    {
        ...
        check ("truncated (missing signature line): unregistered",
               emoboy::verifyLicence (truncated, publicA) == "unregistered");
    }
    ...
    // embedded - it must fail closed, not open.
    {
        ...
        check ("invalid public key: fails closed (unregistered)",
               emoboy::verifyLicence (text, juce::RSAKey()) == "unregistered");
    }
