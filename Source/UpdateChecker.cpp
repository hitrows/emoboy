...
// The one place the manifest URL lives. A plain GET of this static file is
// the entire network footprint - no query parameters, no identifying
// information. Works unauthenticated only because the repo is public
// (2026-08-21 - it was made public specifically so this would resolve;
// raw.githubusercontent.com 404s on an unauthenticated request to a private
// repo, which would make this check permanently, silently useless).
static const char* const kManifestUrl =
    "https://raw.githubusercontent.com/hitrows/emoboy/main/version.json";
...
{
    explicit Fetcher (UpdateChecker& o) : juce::Thread ("EmoBoy update check"), owner (o) {}
...
juce::PropertiesFile::Options UpdateChecker::settingsOptions()
{
    ...
    return o;   // ~/Library/Application Support/Hitrows/EmoBoy/settings.xml on
                // macOS, %APPDATA%\Hitrows\EmoBoy\settings.xml on Windows.
}
...
void UpdateChecker::start (const juce::String& lv)
{
    ...
    // cached version is what a future notice would draw from, so it would
    // keep showing offline and across restarts once a check has found an
    // update - see UpdateChecker.h for why there's no notice UI yet.
    ...
    // The request itself is switchable off (privacy). No UI; would be
    // documented in a README once one exists for this project. Default is
    // on.
