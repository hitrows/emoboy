...
// updates anything - the bundle is memory-mapped while the host has it
// loaded, and installing needs an admin password anyway. Ported from the
// "Not Sure" project's own UpdateChecker (2026-08-21, see its own
// UPDATE-CHECK-SPEC.md for the full reasoning).
//
// One app-lifetime singleton shared across every plugin instance, so a
// session with twenty instances makes at most one request. The result is
// cached in a PropertiesFile with a timestamp; a check under 24h old is
// reused and no request is made. Editors observe it as ChangeListeners and
// query it - nothing here holds an editor pointer, so an editor can close
// while a request is in flight without any dangling reference.
//
// On any failure (offline, DNS, 404, malformed JSON, non-numeric version)
// it does nothing at all: no dialog, no log, no retry loop.
//
// 2026-08-21: this is the backend only - no visual "update available"
// notice is wired into the editor yet (user's own call, they'll add that
// UI later). start()/isUpdateAvailable() work fully regardless.
...
private:
    ...
    juce::String downloadUrl { "https://github.com/hitrows/emoboy/releases/latest" };
