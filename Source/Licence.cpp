...
juce::String verifyLicence (const juce::String& fileText, const juce::RSAKey& publicKey)
{
    ...
    // RSAKey::applyToValue() "dumbly" applies the key regardless of whether
    // it matches the one that signed the value - a corrupted signature or a
    // different key both land here as garbage bytes rather than as a
    // thrown error, so validate before turning them into a String and
    // comparing.
...
{
    // ~/Library/Application Support/Hitrows/EmoBoy/licence.txt on macOS,
    // %APPDATA%\Hitrows\EmoBoy\licence.txt on Windows - see AppPaths.h.
